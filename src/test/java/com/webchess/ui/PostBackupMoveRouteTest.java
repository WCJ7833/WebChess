...
import static com.webchess.ui.GetReplayRoute.ACTIVE_COLOR;
...
public class PostBackupMoveRouteTest {
    ...
    public void setup() {
        ...
        playerSvc1 = gameCenter.newPlayerServices();
        playerSvc1.SignIn("Paul");
        gameCenter.getPlayerLobby().getPlayer("Paul").setColor(Piece.COLOR.RED);
        gameCenter.getPlayerLobby().getPlayer("Paul").setInGame(true);
        gameCenter.getPlayerLobby().getPlayer("Paul").setTurn();


        playerSvc2 = gameCenter.newPlayerServices();
        playerSvc2.SignIn("John");
        gameCenter.getPlayerLobby().getPlayer("John").setColor(Piece.COLOR.WHITE);
        gameCenter.getPlayerLobby().getPlayer("John").setInGame(true);
        gameCenter.getPlayerLobby().getPlayer("John").clearTurn();


        red = gameCenter.getPlayerLobby().getPlayer("John");
        white = gameCenter.getPlayerLobby().getPlayer("Paul");
        board = new BoardView(red,white,red);
        gameCenter.getPlayerLobby().getPlayer("Paul").setBoard(board);
        board = new BoardView(red,white,white);
        gameCenter.getPlayerLobby().getPlayer("John").setBoard(board);
        red = gameCenter.getPlayerLobby().getPlayer("John");
        white = gameCenter.getPlayerLobby().getPlayer("Paul");
        ...
        when(session.attribute(PostBackupMoveRoute.WHITE_PLAYER)).thenReturn(white);
        when(session.attribute(PostBackupMoveRoute.ACTIVE_COLOR)).thenReturn("RED");
        when(session.attribute(PostBackupMoveRoute.BOARD)).thenReturn(board);
        // create a unique CuT for each test
        CuT = new PostBackupMoveRoute(engine,gameCenter,gson);
    }
...
//    @Test
    public void emptyMove() {
        ...
        //gameCenter.getPlayerLobby().getSession().attribute(ACTIVE_COLOR,"RED");
        // Invoke the test
